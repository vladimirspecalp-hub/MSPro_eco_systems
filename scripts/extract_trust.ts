...
try {
    ...
    const traverse = (obj) => {
        ...
        if (typeof obj === 'string') {
            ...
            // Keywords for certificates (generic)
            if (/сертифи[а-я]+/i.test(obj)) {
                // Just flagging context if needed, but for now specific numbers are better
            }
    ...
    const result = {
        ...
        certificates: Array.from(signals.certificates), // Might be empty if no specific cert numbers found
